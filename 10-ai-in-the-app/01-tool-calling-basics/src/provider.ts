...
export function resolveProvider(): AiProvider {
  ...
  if (!isProvider(provider)) {
    throw new Error('AI_PROVIDER muss auf openai, anthropic oder google gesetzt sein (siehe lokale .env.example).')
  }
  if (!process.env[API_KEY_VARIABLES[provider]]) {
    throw new Error(`${API_KEY_VARIABLES[provider]} ist nicht gesetzt (siehe lokale .env.example).`)
  }
  return provider
}
...
export function createModel(modelId?: string): LanguageModel {
  ...
  switch (provider) {
    ...
    case 'google':
      return createGoogle({ apiKey: process.env.GOOGLE_GENERATIVE_AI_API_KEY })(id)
  }
}
