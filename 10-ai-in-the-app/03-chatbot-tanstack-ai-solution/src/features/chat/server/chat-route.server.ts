...
const getCart = getCartDef.server((): CartResult => {
  ...
  return loginId ? toolHandlers.getCart(loginId) : LOGIN_REQUIRED
})

const addToCart = addToCartDef.server(async (input): Promise<CartResult> => {
  const loginId = currentLoginId()
  return loginId ? toolHandlers.addToCart(loginId, input) : LOGIN_REQUIRED
})

const removeFromCart = removeFromCartDef.server((input): CartResult => {
  const loginId = currentLoginId()
  return loginId ? toolHandlers.removeFromCart(loginId, input) : LOGIN_REQUIRED
})

const checkout = checkoutDef.server((): CheckoutToolResult => {
  const loginId = currentLoginId()
  return loginId ? toolHandlers.checkout(loginId) : LOGIN_REQUIRED
})
