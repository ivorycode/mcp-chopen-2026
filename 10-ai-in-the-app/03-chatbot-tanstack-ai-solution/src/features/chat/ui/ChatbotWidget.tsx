...
const CART_TOOLS = new Set(['addToCart', 'removeFromCart', 'checkout'])
...
export default function ChatbotWidget({
  ...
  useEffect(() => {
    ...
    for (const message of messages) {
      for (const part of message.parts) {
        ...
        } else if (CART_TOOLS.has(part.name)) {
          const output = part.output as { ok: boolean } | undefined
          if (output?.ok) cartChanged = true
        }
      }
    }
  ...
  async function submitPrompt(prompt: string) {
    ...
    const approval =
      !isScreenMode &&
      interrupts.find(
        (item) => item.kind === 'tool-approval' && item.canResolve,
      )
    if (approval && approval.kind === 'tool-approval') {
      const answer = text
      ...
      if (answer !== 'ja' && answer !== 'nein') {
        setActionError(
          'Bitte bestätigen Sie mit Ja oder lehnen Sie mit Nein ab.',
        )
        return
      }
      setActionError(null)
      setInput('')
      await approval.resolveInterrupt(answer === 'ja')
      return
    }
          ...
          <div className="chatbot-transcript" ref={scrollRef}>
            <div className="chatbot-transcript-inner">
              ...
              {interrupts.map((interrupt) =>
                interrupt.kind === 'tool-approval' && interrupt.canResolve ? (
                  !isScreenMode ? (
                    <p key={interrupt.id} className="chatbot-system-note">
                      Bestellung jetzt abschicken? Antworten Sie mit Ja oder
                      Nein.
                    </p>
                  ) : (
                    <section
...
function ToolCallView({
  ...
  switch (part.name) {
    ...
    case 'removeFromCart':
      return (
        <CartView
          output={part.output as ToolOutputs['getCart']}
          actions={actions}
        />
      )
    case 'checkout':
      return <CheckoutView output={part.output as ToolOutputs['checkout']} />
    default:
      return null
  }
}
    ...
    <section className="chatbot-summary-card">
      <div className="chatbot-summary-head">
        <div>
          ...
          <strong>
            {output.totalItems}{' '}
            {output.totalItems === 1 ? 'Position' : 'Positionen'}
          </strong>
        </div>
        ...
        {output.items.length ? (
          output.items.map((item) => (
            <div key={item.articleNumber} className="chatbot-summary-item">
              <div className="chatbot-summary-item-content">
                <div className="chatbot-summary-thumb">
                  {getArticleImageUrl(item.celumId) ? (
                    <img
                      src={getArticleImageUrl(item.celumId) ?? undefined}
                ...
                <div>
                  ...
                  <p>
                    ...
                    {item.sellAmount ? ` · ${item.sellAmount}` : ''}
                    {item.sellUnit ? ` ${item.sellUnit}` : ''}
                  </p>
                </div>
              </div>
...
function CheckoutView({ output }: { output: CheckoutToolResult }) {
  if (!output.ok) return <ErrorNote error={output.error} />
