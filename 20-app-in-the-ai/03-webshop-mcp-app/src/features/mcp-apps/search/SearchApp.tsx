...
export function SearchApp() {
  ...
  const [loginId] = useState<string | null>(null)
  ...
  const showSearchResult = (data: SearchPayload) => {
    ...
    // TODO Schritt 3: loginId aus dem Suchresultat übernehmen.
    setMessage(null)
  }
  ...
  const { app, error: connectError } = useApp({
    ...
    onAppCreated: (createdApp) => {
      ...
      createdApp.ontoolinput = (params) => {
        ...
        // TODO Schritt 3: loginId aus params.arguments übernehmen.
      }
  ...
  const addToCart = async (articleNumber: string, quantity: number) => {
    ...
    setMessage(null)
    try {
      // TODO Schritt 4: app.callServerTool mit addToCart und den Argumenten aufrufen.
      const toolResult = {
        isError: true,
        content: [{ type: 'text' as const, text: 'Noch nicht implementiert.' }],
      }
