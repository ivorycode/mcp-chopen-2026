...
test('real Streamable HTTP exposes the account-based MCP tools', async (t) => {
  ...
  if (process.env.EXERCISE_COMPLETE) {
    ...
    const cartUi = await client.readResource({
      uri: 'ui://webshop/cart-ui.html',
    })
    assert.match(cartUi.contents[0]?.text ?? '', /Webshop Warenkorb/)
  } else {
    // Resource-Registrierung darf während der Übung schrittweise ergänzt werden.
    const resources = (await client.listResources()).resources
    assert.ok(
      resources.every(({ uri }) =>
        ['ui://webshop/search-ui.html', 'ui://webshop/cart-ui.html'].includes(
          uri,
        ),
      ),
    )
